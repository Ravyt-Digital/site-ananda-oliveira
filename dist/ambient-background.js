...
(function () {
  ...
  if (!gl) { canvas.hidden = true; canvas.parentElement.classList.add('no-webgl'); return; }
  ...
  const resize = () => {
    // Keep the fragment shader affordable even on high-density screens.
    const scale = Math.min(1, Math.sqrt(650000 / (innerWidth * innerHeight)));
  ...
  const fsSource = `
    ...
    void main() {
        ...
        vec3 color = vec3(0.045, 0.027, 0.055);

        vec3 col1 = vec3(0.87, 0.35, 0.58); // Rosa Ananda
        vec3 col2 = vec3(0.83, 0.72, 0.68); // Champanhe
        vec3 col3 = vec3(0.34, 0.14, 0.43); // Ameixa
  ...
  const createShader = (type, source) => {
    ...
    if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
      ...
      canvas.hidden = true; canvas.parentElement.classList.add('no-webgl');
  ...
  const activeSections = ['inicio', 'metodo', 'contato'].map(id => document.getElementById(id));
  const inView = new Set();
  function shouldRun() { return visible && !document.hidden && !reduced.matches; }
  function updateLoop() {
    if (!shouldRun()) { cancelAnimationFrame(animationFrameId); animationFrameId = 0; return; }
  ...
  if ('IntersectionObserver' in window) {
    const observer = new IntersectionObserver(entries => {
      ...
      visible = inView.size > 0;
      updateLoop();
    }, { threshold: 0 });
    activeSections.forEach(section => section && observer.observe(section));
  }
